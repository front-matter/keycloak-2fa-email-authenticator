...
class ConditionalEmailAuthenticatorFormTest {
    ...
    void setUp() {
        ...
        // Mock User and Realm to avoid NPEs in other checks (though we focus on header
        // check)
        ...
        org.keycloak.sessions.AuthenticationSessionModel session = mock(
                org.keycloak.sessions.AuthenticationSessionModel.class);
        ...
        org.keycloak.email.EmailTemplateProvider emailTemplateProvider = mock(
                org.keycloak.email.EmailTemplateProvider.class);
        when(keycloakSession.getProvider(org.keycloak.email.EmailTemplateProvider.class))
                .thenReturn(emailTemplateProvider);

        // Mock EventBuilder
        org.keycloak.events.EventBuilder eventBuilder = mock(org.keycloak.events.EventBuilder.class);
        when(context.getEvent()).thenReturn(eventBuilder);
        when(eventBuilder.user(any(UserModel.class))).thenReturn(eventBuilder);

        // Mock Execution
        org.keycloak.models.AuthenticationExecutionModel execution = mock(
                org.keycloak.models.AuthenticationExecutionModel.class);
    ...
    void testForceOtpForHttpHeaderMatch() {
        ...
        // Since we are mocking context, calling authenticate will call
        // protected/private methods.
        // But authenticate calls super.authenticate (which calls challenge) if it
        // decides to SHOW_OTP.
        // We can verify behavior by spying or checking side effects.
        // If it decides to SHOW_OTP, it calls super.authenticate(context).
        // Since super.authenticate is an instance method of the class we are testing,
        // we can partial mock it or just let it run.
        // super.authenticate calls context.challenge(...).

        // However, this test class inherits from AbstractUsernameFormAuthenticator
        // which we cannot easily mock the super call of the CUT.
        // But we can verify if context.success() was called (SKIP) or not.

        // Actually, let's just test the logic indirectly.
        // If match, it returns SHOW_OTP -> calls showOtpForm -> super.authenticate ->
        // challenge.
        ...
        // So effectively FORCE and ABSTAIN behave the same (SHOW OTP), unless there is
        // a default outcome config.
        ...
        // It should call challenge (from super.authenticate -> generateAndSendEmailCode
        // -> ... context.challenge)
        // But generateAndSendEmailCode uses session, etc. mocking might be heavy.

        // Maybe we should just test exception handling for now, as that is the specific
        // TODO.
    }
    ...
    void testInvalidRegexPattern() {
        ...
        // New behavior: Should catch exception, log error, and return false (ABSTAIN),
        // thus falling back to DEFAULT (skip)
        // So authenticate() should complete without exception, and call success()
        // because default is skip.
